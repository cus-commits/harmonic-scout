...
function RepoCard({ repo, addFavorite, isFavorited }) {
  ...
  return (
    <div className={`rounded-xl border p-3.5 space-y-2 fade-in ${sig === 'HIGH' ? 'border-sm/40 bg-sm/5' : sig === 'MEDIUM' ? 'border-accent/30 bg-accent/5' : 'border-border/40 bg-surface/60'}`}>
      <div className="flex items-start gap-3">
        ...
        {addFavorite && (
          <button onClick={() => { const s = isFavorited && isFavorited(repo.name); if (!s) addFavorite({ name: repo.name, description: repo.description, website: repo.url }); }}
            className={`flex-shrink-0 text-base ${isFavorited && isFavorited(repo.name) ? 'text-bo' : 'text-muted/40 hover:text-bo'}`}>
      ...
      <div className="flex flex-wrap gap-1.5">
        {repo.language && (
          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md border bg-bo/15 text-bo border-bo/25">
            {repo.language}
          </span>
        )}
        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md border bg-accent/15 text-accent border-accent/25">
          ⭐ {repo.stars}
        </span>
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        <RemoveMenu company={{ name: repo.harmonic?.name || repo.name, website: repo.url }} />
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen max-w-[1080px] mx-auto px-7 pt-6 pb-28 fade-in">
      <div className="mb-6 text-center">
        <h1 className="text-xl font-bold tracking-tight">
          <span className="font-serif italic font-semibold text-accent">dc</span> GitHub Scanner
        </h1>
