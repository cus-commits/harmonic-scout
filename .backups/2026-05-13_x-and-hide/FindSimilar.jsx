...
import RemoveMenu, { isLocallyHidden } from './RemoveMenu';

const API_BASE = import.meta.env.VITE_API_URL || 'https://pigeon-api.up.railway.app';
const userHeaders = () => {
  const u = (typeof window !== 'undefined' ? localStorage.getItem('crm_user') : '') || '';
...
function CompanyMiniCard({ company, addFavorite, isFavorited }) {
  ...
  const [removed, setRemoved] = useState(() => isLocallyHidden(name));
  ...
  return (
    <div className="flex items-start gap-2 px-2.5 py-2 rounded-lg hover:bg-boro/5 transition-colors border border-transparent hover:border-boro/10">
      ...
      <RemoveMenu company={company} onHidden={() => setRemoved(true)} />
    </div>
  );
}
...
function DeepSearchButton({ company }) {
  ...
  const handleClick = (e) => {
    e.stopPropagation();
    const params = new URLSearchParams({ baselineName: company.name || '' });
    if (company.id) params.set('baselineId', company.id);
    if (company.logo_url) params.set('baselineLogo', company.logo_url);
    navigate(`/super?${params.toString()}`);
  };
  return (
...
export default function FindSimilar({ companyId, companyName, addFavorite, isFavorited }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      let companies = [];
      if (companyId) {
        const r = await fetch(`${API_BASE}/api/harmonic/similar/${companyId}?size=100`, { headers: userHeaders() });
        const data = await r.json();
        companies = Array.isArray(data) ? data : (data.companies || []);
        if (data.error) setError(data.error);
      }
      if (companies.length === 0 && companyName) {
        const r = await fetch(`${API_BASE}/api/harmonic/find-similar-by-name?name=${encodeURIComponent(companyName)}&size=100`, { headers: userHeaders() });
        if (r.ok) {
          const data = await r.json();
          companies = Array.isArray(data) ? data : (data.companies || []);
          if (data.error) setError(data.error);
