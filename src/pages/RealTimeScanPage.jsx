...
export default function RealTimeScanPage() {
  ...
  return (
    <div className="min-h-screen max-w-5xl mx-auto px-4 sm:px-6 pt-6 pb-20">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-bright flex items-center gap-2">
              <span className="text-emerald-400">⚡</span> Real Time Scan View
            </h1>
        ...
        <div className="text-right">
          {isRunning ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono text-emerald-400/70">{mins}:{String(secs).padStart(2, '0')}</span>
            </div>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-ink/40 border border-border/15 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/10 bg-black/30">
              <span className="text-[9px] uppercase tracking-widest text-emerald-400/50 font-bold flex items-center gap-1.5">
            ...
            <div ref={feedRef} className="max-h-[400px] overflow-y-auto px-4 py-2 space-y-0.5 font-mono">
              ...
              {feed.map((f, i) => {
                ...
                return (
                  ...
                  } ${isWinner ? 'text-emerald-300/70' : ''} ${isBatch ? 'mt-1 pt-1 border-t border-border/5' : ''}`}>
                ...
                return (
                  <div key={`${batch.phase}-${batch.batchNum}`}
                    ...
                    <div className={`flex items-center justify-between px-3 py-2 border-b border-${phaseColor}-400/10 bg-${phaseColor}-500/[0.03]`}>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] text-emerald-400/50">{winners.length} passed</span>
                    ...
                    {winners.length > 0 && (
                      <div className="p-2.5 space-y-1">
                        {winners.map((c, j) => {
                          ...
                          return (
                            <div key={j} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-emerald-500/[0.04] border border-emerald-400/8 hover:bg-emerald-500/[0.08] transition-all group">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[10px] font-bold text-bright/75">{c.name}</span>
                                  {c.score && <span className="text-[9px] px-1 py-0.5 rounded bg-emerald-500/15 text-emerald-300 font-bold">{c.score}/10</span>}
                                  {c.funding_stage && <span className="text-[8px] text-muted/25">{c.funding_stage}</span>}
                                  {c.funding_total > 0 && <span className="text-[8px] text-sky-400/40">💰 {moneyFmt(c.funding_total)}</span>}
                                </div>
                                {c.reason && <p className="text-[9px] text-muted/40 truncate mt-0.5">{c.reason}</p>}
                                {c.keySignal && <p className="text-[9px] text-amber-400/50 truncate">💡 {c.keySignal}</p>}
                              </div>
                              <div className="flex items-center gap-1 flex-shrink-0">
                                {hLink && (
                                  <a href={hLink} target="_blank" rel="noopener"
                                    className="text-[8px] px-1.5 py-1 rounded bg-violet-500/10 text-violet-400/60 border border-violet-400/15 hover:bg-violet-500/20 transition-all">
                                    Ⓗ Harmonic
                                  </a>
                                )}
                                {c.website && (
        ...
        <div className="space-y-4">
          <div className="bg-ink/40 border border-emerald-400/15 rounded-xl overflow-hidden sticky top-4">
            <div className="px-4 py-2.5 border-b border-emerald-400/10 bg-emerald-500/[0.03]">
              <span className="text-[9px] uppercase tracking-widest text-emerald-400/60 font-bold">
            ...
            <div className="max-h-[70vh] overflow-y-auto p-2.5 space-y-1">
              ...
              {allWinners.map((c, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg hover:bg-emerald-500/[0.04] transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1">
                        ...
                        {c.score && <span className="text-[8px] text-emerald-400/60 font-bold flex-shrink-0">{c.score}/10</span>}
                      </div>
                      {c.reason && <p className="text-[8px] text-muted/30 truncate">{c.reason}</p>}
                    </div>
                    {hLink && (
                      <a href={hLink} target="_blank" rel="noopener"
                        className="text-[7px] px-1 py-0.5 rounded bg-violet-500/10 text-violet-400/50 border border-violet-400/10 hover:bg-violet-500/20 flex-shrink-0 opacity-60 group-hover:opacity-100 transition-all">
                        Ⓗ
                      </a>
                    )}
                  </div>
          ...
          <div className="bg-ink/30 border border-border/15 rounded-xl p-3 space-y-2">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-[10px]">
                <span className="text-violet-400/60">⚡ Pre-Screen</span>
              ...
              <div className="flex items-center justify-between text-[10px]">
                <span className="text-pink-400/60">🎯 Scoring</span>
              ...
              <div className="flex items-center justify-between text-[10px]">
                <span className="text-emerald-400/60">🧠 Deep Analysis</span>
