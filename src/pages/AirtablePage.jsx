  ...
  return (
    <div className="px-5 pt-5 pb-28 max-w-[1080px] mx-auto fade-in">
      ...
      {loading ? (
        ...
        <div>
          {/* Pipeline Header — per FIX_MANUAL Section 6 */}
          <div className="flex items-baseline gap-2.5 mb-3 pb-2.5 border-b border-border/[0.06]">
            <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-muted/50">Pipeline</span>
            <span className="text-[11px] text-muted/30">{filtered.length} companies</span>
            {fundingLoading && <span className="text-[9px] text-amber-400/55 flex items-center gap-1 ml-auto"><span className="w-2.5 h-2.5 border border-amber-400 border-t-transparent rounded-full animate-spin" /> Loading...</span>}
          </div>
              ...
              return (
                <div key={c.airtable_id || i} className="rounded-[14px] border border-border/[0.06] hover:border-accent/18 transition-all relative shadow-sm hover:shadow-md" style={{ background: 'rgba(46, 42, 37, 0.55)' }}>
                  <button
                    ...
                    className="absolute top-3 right-3 text-[10px] px-2.5 py-1 rounded-[6px] border border-border/20 bg-ink/40 text-muted/70 hover:border-accent/30 hover:text-accent font-semibold transition-colors z-10"
