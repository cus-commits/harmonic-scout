...
import {
  ...
  SaveButton, RunButton, RunStatusLine, EnableToggle,
} from './weeklyPanels';
...
function EtaLine({ partner }) {
  ...
  return (
    <p className="text-[11px] font-mono">
      ...
      <span className="text-muted/60"> · top 5 → DD</span>
    </p>
  );
}
...
function HistoryEntry({ entry }) {
  ...
  const t = statusTone(entry.status);
  const summary = [
    formatDurationSec(entry.durationSec),
    entry.budgetUsed ? `$${entry.budgetUsed}` : null,
    entry.corpusSize ? `${Number(entry.corpusSize).toLocaleString()} pool` : null,
    entry.status === 'done' ? `${entry.ddPushed ?? 0} → DD` : null,
  ].filter(Boolean).join(' · ');
  return (
    <div className="border border-border/25 rounded-lg bg-ink/40">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
      ...
      >
        ...
        <span className="text-[11.5px] font-mono text-bright/85">{formatHistoryDate(entry.startedAt)}</span>
        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded border border-border/30 text-muted/70">
          {entry.trigger === 'auto' ? 'auto' : 'manual'}
        </span>
        {entry.status !== 'done' && <span className={`text-[10.5px] font-mono ${t.text}`}>{t.label}{entry.error ? ` — ${entry.error}` : ''}</span>}
        <span className="ml-auto text-[10.5px] font-mono text-muted/60">{summary}</span>
        <span className="text-[10px] text-muted/50 font-mono">{open ? '▾' : '▸'}</span>
      </button>
      {open && (
        <div className="px-3 pb-2.5 space-y-1">
          {(entry.topResults || []).length === 0 ? (
            <p className="text-[11px] text-muted/60 py-1">No results recorded for this run.</p>
          ) : (
            entry.topResults.map((r, i) => (
              <div key={i} className="flex items-center gap-2 py-1 border-t border-border/10 first:border-t-0">
                <span className={`text-[10.5px] font-mono font-bold w-8 text-center rounded border px-1 py-0.5 ${
                  (r.score ?? 0) >= 7 ? 'border-emerald-500/40 text-emerald-400' : 'border-border/40 text-bright/70'
                }`}>{r.score ?? '—'}</span>
                {r.website ? (
                  <a href={r.website.startsWith('http') ? r.website : `https://${r.website}`} target="_blank" rel="noreferrer"
                     className="text-[11.5px] font-semibold text-bright/90 hover:text-accent truncate max-w-[180px]">{r.name}</a>
                ) : (
                  <span className="text-[11.5px] font-semibold text-bright/90 truncate max-w-[180px]">{r.name}</span>
                )}
                {r.pushedToDD && (
                  <span className="text-[9.5px] font-mono font-bold px-1.5 py-0.5 rounded-full border border-boro/40 text-boro bg-boro/10 shrink-0">DD</span>
                )}
                <span title={r.analysis} className="text-[10.5px] text-muted/65 truncate flex-1 min-w-0">{r.analysis}</span>
              </div>
            ))
          )}
        </div>
...
function HistoryPanel({ partnerId, runStatus }) {
  const [entries, setEntries] = useState(null); // null = loading
  useEffect(() => {
    let alive = true;
    setEntries(null);
    fetchPartnerHistory(partnerId).then(h => { if (alive) setEntries(h); });
    return () => { alive = false; };
    // Refetch when a run finishes so the new entry shows up immediately.
  }, [partnerId, runStatus === 'done']);

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[11px] uppercase tracking-wider text-muted/70 font-mono font-semibold">Run history</p>
        <a href="/toppicks" className="text-[10.5px] font-mono text-boro hover:underline">
          Top 5 of every run auto-push to Top Picks (DD) →
        </a>
      </div>
      {entries === null ? (
        <p className="text-[11px] text-muted/60">Loading history…</p>
      ) : entries.length === 0 ? (
        <p className="text-[11px] text-muted/60">No runs yet. History appears here after the first weekly (or manual) run.</p>
      ) : (
        <div className="space-y-1.5">
          {entries.map(e => <HistoryEntry key={e.scanId || e.startedAt} entry={e} />)}
        </div>
      )}
    </div>
  );
...
function ContextEmphasis({ partner, partnerId, controller }) {
  ...
  const rolled = formatRolledRelative(partner?.lastWeightRollAt);
  ...
  async function handlePreview() {
    ...
    if (!out) return;
    // Backend shape: { chunks: [{ id, previewWeight, ... }] }.
    const map = {};
    for (const c of (out.chunks || [])) {
      if (c && c.id != null && c.previewWeight != null) map[c.id] = c.previewWeight;
    }
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-[200px,1fr] gap-5">
      ...
      <aside className="space-y-1">
        ...
        {WEEKLY_PARTNERS.map(p => {
          ...
          const enabled = !!c.partners[p.id]?.isEnabled;
          return (
            <button
              key={p.id}
      ...
      <main className="space-y-4">
        <header className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            <EnableToggle enabled={!!a.isEnabled} onChange={(next) => c.setEnabled(next)} />
              ...
              <button
                ...
                className={`text-[11.5px] font-semibold px-3 py-1.5 rounded-lg border transition-colors ${
                  showHistory
                    ...
                    : 'border-border/30 text-bright/75 hover:border-accent/40 hover:text-accent'
                }`}
              >
                History
              </button>
        ...
        {showHistory && (
          <div className="border border-border/25 rounded-xl bg-card/30 p-4">
            <HistoryPanel partnerId={c.activeId} runStatus={c.runState.status} />
          </div>
        )}
      </main>
    </div>
