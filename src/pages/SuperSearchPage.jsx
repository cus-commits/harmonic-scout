  ...
  return (
    <button
      ...
      className={`${base} rounded-lg border font-medium transition-all duration-150 ${
        active
          ? 'bg-sky-500/12 border-sky-400/35 text-sky-300 shadow-[0_0_8px_rgba(56,189,248,0.08)]'
...
const sourceColors = {
  twitter: 'bg-white/8 text-white/90 border-white/15',
  farcaster: 'bg-purple-500/12 text-purple-300 border-purple-400/25',
  github: 'bg-gray-500/12 text-gray-300 border-gray-400/25',
  producthunt: 'bg-orange-500/12 text-orange-300 border-orange-400/25',
  harmonic: 'bg-amber-500/12 text-amber-300 border-amber-400/25',
};

const signalStyles = {
  HIGH: { card: 'border-emerald-500/25 shadow-[0_0_12px_rgba(16,185,129,0.06)]', badge: 'bg-emerald-500/12 text-emerald-400 border-emerald-500/25', dot: '🟢' },
  MEDIUM: { card: 'border-amber-500/20 shadow-[0_0_10px_rgba(245,158,11,0.04)]', badge: 'bg-amber-500/12 text-amber-400 border-amber-500/25', dot: '🟡' },
  LOW: { card: 'border-border/15', badge: 'bg-red-500/8 text-red-400/65 border-red-500/15', dot: '🔴' },
};
  ...
  return (
    <div className={`bg-surface/50 backdrop-blur-sm border rounded-xl p-3.5 space-y-2.5 transition-all ${style.card}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap mb-1">
            ...
            {signal.companyName && (
              <span className="text-[9px] px-1.5 py-0.5 rounded bg-sky-500/8 text-sky-300 border border-sky-400/20 font-medium">
                {signal.companyName}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {addFavorite && (
            <button onClick={() => !saved && addFavorite({ name: companyName, description: signal.text?.slice(0, 200), website: webUrl })}
              className={`text-base ${saved ? 'text-sky-400' : 'text-muted/40 hover:text-sky-400'}`}>
              {saved ? '★' : '☆'}
            </button>
          )}
          <div className="relative">
            <button onClick={() => setMenuOpen(!menuOpen)} className="text-muted/40 hover:text-red-400 transition-colors">
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute top-full right-0 mt-1 z-50 bg-[#1a1d2e] border border-border/30 rounded-lg shadow-xl py-1 min-w-[140px]">
                  <button onClick={handleHide} className="w-full text-left px-3 py-2 text-[11px] text-muted/60 hover:text-bright hover:bg-white/5 transition-colors">👁‍🗨 Hide for me</button>
                  <button onClick={handleBackburn} className="w-full text-left px-3 py-2 text-[11px] text-red-400/70 hover:text-red-400 hover:bg-red-500/5 transition-colors border-t border-border/10">🔥 Backburn</button>
                </div>
              </>
            )}
          </div>
      ...
      <div className="flex items-center gap-3 text-[10px] text-muted/60 flex-wrap">
        ...
        {signal.meta?.webGrowth && <span className={signal.meta.webGrowth > 0 ? 'text-emerald-400/80' : 'text-red-400/65'}>🌐 {signal.meta.webGrowth > 0 ? '+' : ''}{signal.meta.webGrowth}%</span>}
        {signal.meta?.hcGrowth && <span className={signal.meta.hcGrowth > 0 ? 'text-emerald-400/80' : 'text-red-400/65'}>📈 HC {signal.meta.hcGrowth > 0 ? '+' : ''}{signal.meta.hcGrowth}%</span>}
        {signal.meta?.engGrowth && <span className={signal.meta.engGrowth > 0 ? 'text-emerald-400/80' : 'text-red-400/65'}>⚙️ Eng {signal.meta.engGrowth > 0 ? '+' : ''}{signal.meta.engGrowth}%</span>}
        {signal.timestamp && <span className="ml-auto text-bright/40">{timeSince(signal.timestamp)}</span>}
      </div>
      <div className="flex items-center gap-2">
        {signal.url && (
          <a href={signal.url} target="_blank" rel="noopener" className="text-[10px] text-sky-400/70 hover:text-sky-300 hover:underline truncate transition-colors">
            ↗ {signal.url.replace(/^https?:\/\//, '').slice(0, 45)}
          </a>
        )}
        {(signal.companyName || signal.source === 'harmonic') && <div className="flex items-center gap-1.5 flex-wrap">{signal.source === 'harmonic' && signal.id && <a href={`/company/${signal.id.replace('hm-','')}`} className="text-[8px] px-1.5 py-0.5 rounded bg-pink-500/10 text-pink-400 border border-pink-400/15 hover:bg-pink-500/20 font-bold" title="Company Card">H</a>}<FindSimilar addFavorite={addFavorite} isFavorited={isFavorited} companyId={signal.id?.replace('hm-','')} companyName={signal.companyName || signal.title} /><CrmButton company={{ name: signal.companyName || signal.title, website: signal.url }} /></div>}
      </div>
    </div>
  );
}
...
function SectionHeader({ title, subtitle, onToggleAll, allSelected }) {
  return (
    <div className="flex items-center justify-between">
      ...
      {onToggleAll && (
        <button onClick={onToggleAll} className="text-[10px] text-sky-400/70 hover:text-sky-300 transition-colors">
  ...
  return (
    <button onClick={onClick}
      className={`text-[10px] px-2 py-0.5 rounded-full border transition-all duration-150 ${
        active ? (color || 'bg-sky-500/10 border-sky-400/30 text-sky-300') : 'border-border/20 text-muted/40 hover:border-border/35'
      }`}>
      {label}
    </button>
  );
    ...
    <div className="min-h-screen max-w-4xl mx-auto px-8 pt-4 pb-24">
      <div className="mb-5 text-center">
        <h1 className="text-xl font-extrabold tracking-tight">
          ...
          <span style={{ background: 'linear-gradient(135deg, #38bdf8 0%, #818cf8 50%, #e11d48 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Super Search
          </span>
        </h1>
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="relative flex-1">
          <button onClick={() => setShowDropdown(!showDropdown)} className="w-full flex items-center justify-between bg-ink/60 border border-border/25 rounded-lg px-3 py-2 text-xs text-muted/70 hover:border-sky-400/25 transition-colors">
            <span className="flex items-center gap-1.5">
              <span className="opacity-40">📂</span>
              {activeSearchName ? (
                <span className="text-sky-300 font-medium">{activeSearchName} <span className="text-muted/30">(editing)</span></span>
              ) : (
          ...
          {showDropdown && savedSearches.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-surface border border-border/30 rounded-xl shadow-2xl shadow-black/40 z-20 max-h-60 overflow-y-auto">
              {savedSearches.map(s => (
                <button key={s.name} onClick={() => handleLoadSearch(s)} className={`w-full flex items-center justify-between px-3 py-2.5 text-xs hover:bg-sky-500/5 transition-colors group border-b border-border/15 last:border-0 ${activeSearchName === s.name ? 'bg-sky-500/5' : ''}`}>
                  ...
                  <span onClick={(e) => handleDeleteSearch(s.name, e)} className="text-muted/40 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all ml-2 text-sm flex-shrink-0">✕</span>
                </button>
              ))}
            </div>
          )}
        ...
        {!showSaveInput && !showOverridePrompt ? (
          <button onClick={handleSaveSearch} className="flex-shrink-0 px-3 py-2 rounded-lg border border-sky-400/20 text-sky-400/80 text-xs font-medium hover:bg-sky-500/8 transition-colors">
            {activeSearchName ? 'Update' : 'Save'}
          </button>
        ) : showOverridePrompt ? (
          <div className="flex gap-1.5 flex-shrink-0">
            <button onClick={handleOverrideSave} className="px-2.5 py-2 rounded-lg bg-sky-500 text-white text-xs font-bold shadow-md shadow-sky-500/20">Update "{activeSearchName}"</button>
          ...
          <div className="flex gap-1.5 flex-shrink-0">
            <input type="text" value={saveName} onChange={e => setSaveName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSaveSearch()} placeholder="Name..." className="w-28 bg-ink/60 border border-border/30 rounded-lg px-2 py-2 text-xs text-bright outline-none focus:border-sky-400/40" autoFocus />
            <button onClick={handleSaveSearch} className="px-2.5 py-2 rounded-lg bg-sky-500 text-white text-xs font-bold shadow-md shadow-sky-500/20">Save</button>
      ...
      <div className="space-y-4 bg-ink/30 border border-border/15 rounded-2xl p-4 mb-4 shadow-inner shadow-black/10">
        ...
        <div className="space-y-2">
          ...
          <input type="text" value={customKeywords} onChange={e => setCustomKeywords(e.target.value)}
            ...
            className="w-full bg-ink/50 border border-border/25 rounded-lg px-3 py-2.5 text-xs text-bright outline-none focus:border-sky-400/35 transition-colors placeholder:text-muted/40" />
        </div>
      </div>
        ...
        {[
          ...
          <button key={t.id} onClick={() => setSuperTier(t.id)}
            className={`text-[10px] px-3 py-1.5 rounded-lg border transition-all ${
              superTier === t.id ? 'bg-sky-500/12 border-sky-400/30 text-sky-300 font-bold' : 'border-border/20 text-muted/40 hover:border-border/40'
            }`} title={t.desc}>
      ...
      {showCostConfirm && (() => {
        ...
        return (
          <div className={`mb-4 bg-ink/40 border rounded-xl p-4 space-y-3 ${isExpensive ? 'border-red-400/30' : 'border-amber-400/20'}`}>
            <p className="text-[10px] text-amber-400/60 uppercase tracking-wider font-bold">Cost Breakdown — Super Search</p>
            {isExpensive && (
              <div className="bg-red-500/10 border border-red-400/25 rounded-lg px-3 py-2">
                <p className="text-[11px] text-red-400 font-bold">⚠️ High cost warning</p>
                <p className="text-[10px] text-red-300/60 mt-0.5">This search is estimated at ~${totalEst.toFixed(2)} due to {sectors.length} sectors × {sources.length} sources. Consider reducing sectors or switching to Sonnet-only tier.</p>
              </div>
            )}
            <div className="space-y-1.5 text-[11px]">
              ...
              <div className="flex justify-between text-muted/50">
                ...
                <span className="text-amber-300/60">~${sonnetCost.toFixed(2)}</span>
              </div>
              {superTier === 'opus20' && (
                <div className="flex justify-between text-muted/50">
                  <span>Opus deep scoring (top 20 signals)</span>
                  <span className="text-amber-300/60">~${(Math.min(20, estForClaude) * 0.015).toFixed(2)}</span>
                </div>
              )}
              {superTier === 'opus80' && (
                <div className="flex justify-between text-muted/50">
                  <span>Opus deep scoring (top 80 signals)</span>
                  <span className="text-amber-300/60">~${(Math.min(80, estForClaude) * 0.015).toFixed(2)}</span>
                </div>
              )}
              <div className="border-t border-border/15 pt-1.5 flex justify-between font-bold">
                ...
                <span className={totalEst > 4 ? 'text-red-400' : 'text-amber-300'}>~${totalEst.toFixed(2)}</span>
              </div>
            ...
            <div className="flex gap-2">
              <button onClick={confirmAndScan}
                className={`flex-1 py-2 rounded-lg font-bold text-[11px] transition-all ${
                  isExpensive ? 'bg-red-500/15 border border-red-400/30 text-red-300 hover:bg-red-500/25' : 'bg-emerald-500/15 border border-emerald-400/30 text-emerald-300 hover:bg-emerald-500/25'
                }`}>
      ...
      <button onClick={handleScan} disabled={isScanning}
        className="w-full py-3.5 rounded-xl font-extrabold text-sm tracking-wide active:scale-[0.98] transition-all duration-200 disabled:opacity-40 mb-5 hover:brightness-110 text-white"
        style={{
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setShowHistory(!showHistory)}
          className={`text-[10px] px-3 py-1.5 rounded-lg border transition-all ${showHistory ? 'bg-sky-500/10 border-sky-400/30 text-sky-300' : 'border-border/20 text-muted/40 hover:border-sky-400/25'}`}>
            ...
            <div key={h.id || i} className="border border-border/10 rounded-lg p-2.5 space-y-1">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 text-[9px]">
                  ...
                  <span className="text-emerald-400/60">⭐ {h.signals} HIGH</span>
                  {h.ddPushed > 0 && <span className="text-violet-400/60">📋 {h.ddPushed} → DD</span>}
                  <span className="text-amber-400/40">💲 ${h.cost}</span>
              ...
              {h.topResults?.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {h.topResults.map((r, j) => (
                    <span key={j} className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-500/8 text-emerald-300/60 border border-emerald-400/15">
                      {r.name} ({r.source})
                    </span>
                  ))}
                </div>
              ...
              {h.analysis && (
                <details className="text-[10px]">
                  <summary className="text-violet-400/50 cursor-pointer hover:text-violet-300">🧠 Analysis</summary>
      ...
      {showSavePrompt && !isScanning && displayResults && !displayResults.error && (
        <div className="mb-4 bg-sky-500/5 border border-sky-400/20 rounded-xl p-3 flex items-center gap-3">
          <p className="text-[11px] text-sky-300/70 flex-1">Save this search for future use?</p>
          <button onClick={() => { setShowSaveInput(true); setShowSavePrompt(false); }}
            className="text-[10px] px-3 py-1 rounded-lg bg-sky-500/15 border border-sky-400/30 text-sky-300 font-medium hover:bg-sky-500/25">Save</button>
      ...
      {/* Scan progress panel */}
      {isScanning && (
        <div className="mb-5 bg-ink/30 border border-sky-400/15 rounded-xl p-4 space-y-3">
            ...
            {['import', 'filter', 'screen', 'done'].map((s, i) => {
              ...
              return (
                <React.Fragment key={s}>
                  ...
                  <span className={`px-2 py-0.5 rounded ${isActive ? 'bg-sky-500/15 text-sky-300 font-bold' : isPast ? 'text-emerald-400/50' : 'text-muted/25'}`}>{labels[s]}</span>
                </React.Fragment>
              );
            })}
          </div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-[9px] text-amber-400/40">Est. cost: ~$0.08-0.15 (Sonnet only)</span>
            <button onClick={cancelSuperSearch} className="text-[9px] px-2 py-1 rounded border border-red-400/25 text-red-400/60 hover:text-red-400 hover:border-red-400/40">Cancel</button>
          </div>
        </div>
      )}
      ...
      {displayResults && (
        <div className="space-y-3">
          {displayResults?.error && (
            <div className="bg-red-500/8 border border-red-500/15 rounded-xl p-3 shadow-sm">
              <p className="text-red-400/90 text-sm">{displayResults?.error}</p>
            </div>
          )}
          {displayResults?.sourceStats && (
            <div className="flex flex-wrap items-center gap-2.5 text-[10px] py-1">
              ...
              {displayResults?.sourceStats.twitter > 0 && <span className="text-white/60">𝕏 {displayResults?.sourceStats.twitter}</span>}
              {displayResults?.sourceStats.farcaster > 0 && <span className="text-purple-400/60">🟪 {displayResults?.sourceStats.farcaster}</span>}
              {displayResults?.sourceStats.github > 0 && <span className="text-gray-400/60">🐙 {displayResults?.sourceStats.github}</span>}
              {displayResults?.sourceStats.producthunt > 0 && <span className="text-orange-400/60">🐱 {displayResults?.sourceStats.producthunt}</span>}
              {displayResults?.sourceStats.harmonic > 0 && <span className="text-amber-400/60">🔮 {displayResults?.sourceStats.harmonic}</span>}
              {displayResults?.elapsed && <><span className="text-border/30">|</span><span className="text-muted/40">⏱ {displayResults?.elapsed}s</span></>}
              {displayResults?.estimatedCost && <><span className="text-border/30">|</span><span className="text-amber-400/40">💲 ~${displayResults?.estimatedCost}</span></>}
              {displayResults?.ddPushed > 0 && <><span className="text-border/30">|</span><span className="text-emerald-400/60">📋 {displayResults?.ddPushed} → DD</span></>}
            </div>
          )}
          {displayResults?.analysis && (
            <div className="bg-sky-500/4 border border-sky-400/12 rounded-xl p-4 shadow-sm">
              <p className="text-[10px] uppercase tracking-widest text-sky-400/55 font-bold mb-1.5">Claude Analysis</p>
