...
export default function NavBar({ onLogout, favCount, nickname, setNickname, userId, addFavorite, isFavorited }) {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (mobilePanelRef.current && !mobilePanelRef.current.contains(e.target)) setMobilePanel(null);
    };
    if (mobilePanel) document.addEventListener('mousedown', handler);
  ...
  const appPaths = APP_ITEMS.map(a => a.path).filter(p => p !== '/');
  const isAppActive = appPaths.includes(location.pathname);
  ...
  const tabs = [
    { path: '/toppicks', label: 'DD', icon: TopPicksIcon },
    { path: '/airtable', label: 'CRM', icon: AirtableNavIcon, badge: (!crmRestricted && pendingCount > 0) ? pendingCount : null },
    { path: '/reachouts', label: 'DMs', icon: ReachoutsIcon, badge: reachoutsUnread > 0 ? reachoutsUnread : null },
    { path: '/favorites', label: 'Favs', icon: StarIcon, badge: favCount },
    { path: '/community', label: 'Team', icon: DealsIcon },
    { path: '/portcos', label: 'Portcos', icon: PortcosIcon },
  ];
  ...
  /* ═══════════════════════════════════════════════════ */
  /* KEY EDIT WIDGET — shared between mobile & desktop   */
  /* ═══════════════════════════════════════════════════ */
  ...
  const SearchTypeahead = ({ onNav }) => (
    <>
                ...
                <div className="flex items-center gap-2">
                  {s.logo_url ? (
                    ...
                    <div className="w-7 h-7 rounded-lg bg-amber-500/10 flex items-center justify-center flex-shrink-0">
                      <span className="text-amber-400 text-[10px] font-bold">{(s.name || '?')[0]}</span>
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <button onClick={() => { onNav(); navigate(`/company/${s.id}`); }}
                      className="text-[11px] font-semibold text-bright hover:text-amber-300 truncate block text-left transition-colors">{s.name}</button>
                    <div className="flex items-center gap-1 flex-wrap">
                      {s.stage && <span className="text-[8px] px-1 rounded bg-emerald-500/10 text-emerald-300/65">{String(s.stage).replace(/_/g,' ')}</span>}
                      {funding && <span className="text-[8px] text-sky-300/65">💰 {funding}</span>}
                      {webUrl && <a href={webUrl} target="_blank" rel="noopener" onClick={e => e.stopPropagation()} className="text-[8px] text-blue-400/65 hover:text-blue-300">🌐</a>}
                    </div>
                  </div>
      ...
      {searchQuery.length >= 2 && searchResults.length === 0 && !searchLoading && (
        <p className="text-muted/40 text-[10px] text-center py-2 border-t border-white/5">No companies found</p>
      )}
      {searchQuery.length >= 2 && (
        <p className="text-[8px] text-muted/40 text-center py-1 border-t border-white/5">Press Enter for full results</p>
      )}
    </>
  );
  ...
  if (isMobile) {
    const crmPages = [
      { path: '/airtable', label: 'CRM Pipeline', icon: '📋', badge: (!crmRestricted && pendingCount > 0) ? pendingCount : null },
      { path: '/reachouts', label: 'Reachouts', icon: '📡', badge: reachoutsUnread > 0 ? reachoutsUnread : null },
      { path: '/community', label: 'Team Votes', icon: '🤝' },
      { path: '/toppicks', label: 'DD Pipeline', icon: '🎯' },
      { path: '/portcos', label: 'Portfolio', icon: '💼' },
      { path: '/favorites', label: 'Favorites', icon: '⭐', badge: favCount || null },
    ];
    ...
    const allPages = [
      ...
      { path: '/producthunt', label: 'Product Hunt', icon: '🐱' },
      { path: '/github', label: 'GitHub', icon: '🐙' },
    ];

    return (
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-surface/95 backdrop-blur-xl border-t border-border/30" style={{ boxShadow: '0 -4px 20px rgba(0,0,0,0.3), 0 -1px 0 rgba(80,227,194,0.08)' }}>

        {/* Mobile flyout panels */}
        {mobilePanel && (
          <div ref={mobilePanelRef} className="absolute bottom-full left-0 right-0 mb-0 bg-[#12141f] border-t border-border/20 max-h-[70vh] overflow-y-auto fade-in"
              ...
              <div className="p-3 space-y-1">
                ...
                <div className="relative mb-2">
                  <input ref={searchInputRef} type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                    ...
                    className="w-full bg-ink/60 border border-border/25 rounded-lg px-3 py-2 text-sm text-bright outline-none focus:border-amber-400/40 placeholder:text-muted/40" />
                  {searchLoading && <div className="absolute right-3 top-2.5 w-3 h-3 border-2 border-amber-400 border-t-transparent rounded-full animate-spin" />}
                </div>

                {searchResults.length > 0 && (
                  <div className="mb-2 rounded-lg border border-white/5 overflow-hidden">
                    <SearchTypeahead onNav={() => setMobilePanel(null)} />
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {allPages.map(p => {
                    ...
                    return (
                      <button key={p.path} onClick={() => { setMobilePanel(null); navigate(p.path); }}
                        className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-left transition-colors ${
                          active ? 'bg-accent/10 text-accent' : 'text-bright/70 hover:bg-white/5'
                        }`}>
            ...
            {mobilePanel === 'crm' && (
              <div className="p-3 space-y-1">
                <p className="text-[10px] uppercase tracking-widest text-muted/50 font-bold mb-2">Pipeline & Team</p>
                {crmPages.map(p => {
                  const active = location.pathname === p.path;
                  return (
                    <button key={p.path} onClick={() => { setMobilePanel(null); navigate(p.path); }}
                      ...
                      {p.badge > 0 && (
                        <span className="min-w-[20px] h-5 px-1.5 rounded-full bg-amber-500 text-ink text-[10px] font-bold flex items-center justify-center">
                          {p.badge}
                        </span>
                      )}
            ...
            {mobilePanel === 'settings' && (
              <div className="p-3 space-y-3">
                <p className="text-[10px] uppercase tracking-widest text-muted/50 font-bold">Settings</p>

                {/* Nickname */}
                <div className="glass-card p-3">
                  <p className="text-[10px] uppercase tracking-wider text-muted font-medium mb-1.5">Your nickname</p>
                  {editingName ? (
                ...
                <div className="glass-card p-3 space-y-1">
                  <p className="text-[10px] uppercase tracking-wider text-muted font-medium mb-1">API Keys</p>
                ...
                {/* Wipe */}
                <div className="glass-card p-3">
                  <p className="text-[10px] uppercase tracking-wider text-muted font-medium mb-1.5">Scan History</p>
                  {wipeConfirm ? (
                    <div className="space-y-2">
                      <p className="text-[11px] text-red-400 leading-relaxed">Clear ALL dismissed companies for every team member. Cannot be undone.</p>
                      <div className="flex gap-2">
                        ...
                        }} className="flex-1 text-xs px-2 py-1.5 rounded-lg bg-red-500/20 text-red-400 border border-red-500/30 font-bold">Yes, Wipe</button>
                      </div>
                    </div>
                  ) : wipeDone ? (
                    <p className="text-xs text-emerald-400 px-2 py-1.5">History cleared</p>
                  ) : (
                ...
                <button onClick={() => { setMobilePanel(null); onLogout(); navigate('/setup'); }}
                  className="w-full text-left text-sm text-red-400 px-3 py-2.5 rounded-lg hover:bg-red-500/10 transition-colors border border-red-500/15">
                  🔌 Disconnect Keys
                </button>
              </div>
            )}
        ...
        <div className="flex items-center justify-around h-14 max-w-md mx-auto px-4">
          <button onClick={() => setMobilePanel(mobilePanel === 'apps' ? null : 'apps')}
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-all ${
              mobilePanel === 'apps' ? 'text-accent' : 'text-muted hover:text-bright'
            }`}>
            <AppsIcon active={mobilePanel === 'apps'} />
            <span className="text-[9px] font-semibold tracking-wide uppercase">Apps</span>
          </button>

          <button onClick={() => setMobilePanel(mobilePanel === 'crm' ? null : 'crm')}
            className={`relative flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-all ${
              mobilePanel === 'crm' ? 'text-accent' : 'text-muted hover:text-bright'
            }`}>
            <AirtableNavIcon active={mobilePanel === 'crm'} />
            <span className="text-[9px] font-semibold tracking-wide uppercase">CRM</span>
            {!crmRestricted && pendingCount > 0 && (
              <span className="absolute top-0.5 right-2 min-w-[16px] h-4 px-1 rounded-full bg-amber-500 text-ink text-[9px] font-bold flex items-center justify-center">
                {pendingCount}
              </span>
            )}
          </button>
          ...
          <button onClick={() => setMobilePanel(mobilePanel === 'settings' ? null : 'settings')}
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-all ${
              mobilePanel === 'settings' ? 'text-accent' : 'text-muted hover:text-bright'
            }`}>
            <SettingsIcon />
            <span className="text-[9px] font-semibold tracking-wide uppercase">Settings</span>
          </button>
        </div>
      </nav>
    );
  ...
  /* ═══════════════════════════════════════════════════ */
  /* DESKTOP NAV — same as before + API key mgmt        */
  /* ═══════════════════════════════════════════════════ */
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-surface/95 backdrop-blur-xl border-t border-border/30" style={{ boxShadow: '0 -4px 20px rgba(0,0,0,0.3), 0 -1px 0 rgba(80,227,194,0.08)', fontSize: 'calc(1rem / 1.1)' }}>
      {/* Settings menu */}
      {showMenu && (
        <div className="absolute bottom-full right-4 mb-2 glass-card p-3 min-w-[240px] fade-in space-y-3 z-[60]">
          ...
          {/* API Keys */}
          <div>
            <p className="text-[10px] uppercase tracking-wider text-muted font-medium mb-1.5">API Keys</p>
            <KeyEditRow type="anthropic" label="Anthropic" placeholder="sk-ant-..." />
          ...
          <div>
            ...
            {wipeConfirm ? (
              <div className="space-y-2">
                <p className="text-[11px] text-red-400 leading-relaxed">This will clear ALL dismissed companies for every team member. Cannot be undone.</p>
                <div className="flex gap-2">
                  <button onClick={() => setWipeConfirm(false)} className="flex-1 text-xs px-2 py-1.5 rounded-lg border border-border/40 text-muted">Cancel</button>
                  <button onClick={async () => {
                    try {
          ...
          <button onClick={() => { setShowMenu(false); onLogout(); navigate('/setup'); }}
            className="w-full text-left text-sm text-red-400 px-2 py-1.5 rounded-lg hover:bg-red-500/10 transition-colors">
            Disconnect Keys
          </button>
        </div>
      )}
      ...
      <div className="flex items-center justify-evenly h-16 max-w-4xl mx-auto relative overflow-visible">
        ...
        <button onClick={() => navigate('/')}
          className={`group relative flex flex-col items-center gap-0 px-1.5 py-1 transition-all duration-200 min-w-0 ${
            location.pathname === '/' ? 'text-accent' : 'text-muted hover:text-bright'
          }`}>
          {location.pathname === '/' && <div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-full bg-accent" />}
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={location.pathname === '/' ? 'text-accent' : ''}>
            <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
          </svg>
          <span className="text-[8px] font-medium tracking-wide uppercase leading-tight">Home</span>
        </button>

        {/* Search */}
        <div ref={searchRef} className="relative">
          <button
            onClick={() => { setShowSearch(!showSearch); setShowApps(false); setShowMenu(false); if (!showSearch) { setSearchQuery(''); setSearchResults([]); } }}
            className={`group relative flex flex-col items-center gap-0 px-1.5 py-1 transition-all duration-200 min-w-0 ${
              showSearch ? 'text-accent' : 'text-muted hover:text-bright'
            }`}>
            {showSearch && <div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-full bg-accent" />}
            <SearchNavIcon active={showSearch} />
            <span className="text-[8px] font-medium tracking-wide uppercase leading-tight">Search</span>
          </button>

          {showSearch && (
            <div className="absolute bottom-full left-0 mb-2 w-[320px] bg-[#1a1d2e] border border-amber-400/25 rounded-xl z-[60]"
        ...
        <div className="relative">
          <button
            onClick={() => { setShowApps(!showApps); setShowMenu(false); }}
            className={`group flex flex-col items-center gap-0 px-1.5 py-1 transition-all duration-200 min-w-0 ${
              isAppActive || showApps ? 'text-accent' : 'text-muted hover:text-bright'
            }`}>
            {isAppActive && <div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-5 h-0.5 rounded-full bg-accent" />}
            <div className={`transition-all duration-200 ${isAppActive ? 'drop-shadow-[0_0_6px_rgba(80,227,194,0.5)]' : ''}`}>
              <AppsIcon active={isAppActive || showApps} />
            </div>
            <span className="text-[8px] font-medium tracking-wide uppercase leading-tight">Apps</span>
          </button>
          {showApps && (
            <div ref={appsRef} className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-surface/95 backdrop-blur-xl border border-border/30 rounded-xl shadow-2xl shadow-black/50 min-w-[200px] overflow-hidden fade-in z-[60]">
              <p className="text-[10px] uppercase tracking-widest text-muted/50 font-bold px-3 pt-2.5 pb-1">Apps</p>
              {APP_ITEMS.map((app) => {
                const active = location.pathname === app.path;
                return (
                  <button key={app.path} onClick={() => { navigate(app.path); setShowApps(false); }}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors ${
                      active ? 'bg-sky-500/8 text-sky-300' : 'text-bright/80 hover:bg-white/5'
                    }`}>
                    <div className={`${active ? 'text-sky-400' : 'text-muted/60'}`}><app.icon active={active} /></div>
        ...
        {/* Settings */}
        <button onClick={() => { setShowMenu(!showMenu); setShowApps(false); }}
          className="group flex flex-col items-center gap-0 px-1.5 py-1 text-muted hover:text-bright transition-all duration-200 min-w-0">
          <SettingsIcon />
          <span className="text-[8px] font-medium tracking-wide uppercase leading-tight">Settings</span>
        </button>
        ...
        {/* Daxos logo */}
        <a href="https://www.daxos.capital" target="_blank" rel="noopener" className="flex-shrink-0 opacity-[0.35] hover:opacity-[0.55] transition-opacity" title="daxos.capital" style={{ fontSize: '115%' }}>
...
function StarIcon({ active }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={active ? 2.2 : 1.6} strokeLinecap="round" strokeLinejoin="round">
...
function DealsIcon({ active }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2 : 1.5} strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M3.5 12.5L7 16l2.5-1 3 3 8-8" />
      <path d="M2 15l3.5-3.5" />
      <path d="M18.5 8.5L22 12" />
    </svg>
  );
}
...
function ProductHuntIcon({ active }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.2 : 1.6}>
...
function TopPicksIcon({ active }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={active ? 1.5 : 1.6} strokeLinecap="round" strokeLinejoin="round">
