    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50"
      ...
      {!crmRestricted && pendingCount > 0 && (
        <button onClick={() => navigate('/airtable')}
          className="fixed top-3 right-3 z-[60] flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent/15 border border-accent/25 hover:bg-accent/20 transition-all shadow-lg shadow-black/30">
          <span className="w-2 h-2 bg-accent rounded-full animate-pulse" />
          <span className="text-[10px] text-accent font-medium">{pendingCount} {pendingCount === 1 ? 'deal needs' : 'deals need'} your vote</span>
        </button>
      )}
    </nav>
  );
...
function StarIcon({ active }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={active ? 2 : 1.6} strokeLinecap="round" strokeLinejoin="round">
...
function AirtableNavIcon({ active }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2 : 1.6} strokeLinejoin="round">
